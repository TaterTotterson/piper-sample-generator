...
For compatibility with those training scripts, this repository keeps the full generator stack (`piper_train`), augmentation tooling/assets, and model metadata JSON files in `models/`.
...
For `.pt` models, make sure the matching `${model}.json` file exists beside the model file.
